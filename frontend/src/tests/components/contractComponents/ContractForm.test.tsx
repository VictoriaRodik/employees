...
vi.mock("../../../utils/contractFormatted", () => ({
  contractFormatted: (contract: ContractInterface) => ({
    ...
    contractEndDate: new Date(contract.contractEndDate).toLocaleDateString(
      "en-CA"
    ),
    passportIssueDate: new Date(contract.passportIssueDate).toLocaleDateString(
      "en-CA"
    ),
  }),
}));
...
describe("ContractForm", () => {
  ...
  it("renders all fields with default values", () => {
    ...
    expect(
      screen.getByLabelText("Дата закінчення контракту")
    ).toBeInTheDocument();
