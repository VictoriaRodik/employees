  ...
  default: ({ open, title, onClose, onSubmit, initialValues }: EmployeeFormModalProps) => (
    open ? (
      <div data-testid="modal">
        <h1>{title}</h1>
        <button onClick={() => onSubmit({ ...initialValues, id: initialValues?.id || 0 })}>
          Submit
        </button>
...
describe('EmployeeList', () => {
  ...
  it('renders employees and components when loaded', () => {
    (useEmployees as any).mockReturnValue({ data: mockEmployees, isLoading: false, error: null });
    render(<EmployeeList />);
  ...
  it('sorts employees by selected field', async () => {
    ...
    fireEvent.change(screen.getByTestId('sort-select'), { target: { value: 'personnelNumber' } });
  ...
  it('deletes employee', () => {
    ...
    (useEmployees as any).mockReturnValue({ data: mockEmployees, isLoading: false, error: null, deleteEmployee });
    render(<EmployeeList />);
  ...
  it('submits new employee', async () => {
    ...
    (useEmployees as any).mockReturnValue({ data: mockEmployees, isLoading: false, error: null, createEmployee });
    render(<EmployeeList />);
  ...
  it('updates existing employee', async () => {
    ...
    (useEmployees as any).mockReturnValue({ data: mockEmployees, isLoading: false, error: null, updateEmployee });
    render(<EmployeeList />);

    fireEvent.click(screen.getAllByText('Edit')[0]);
    fireEvent.click(screen.getByText('Submit'));
    await waitFor(() => {
      expect(updateEmployee.mutate).toHaveBeenCalledWith(expect.objectContaining({ id: 2 }));
    });
  });
});